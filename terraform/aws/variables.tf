...
variable "aws_vpc_id" {
  type        = string
  description = "The AWS VPC ID where the resources will be created. The VPC must already exist."
}

variable "aws_container_subnets" {
  type        = list(string)
  description = "The subnet ids where the containers will run"
}

variable "aws_rds_subnet_ids" {
  type        = list(string)
  description = "The subnet ids where the database will run"
}
